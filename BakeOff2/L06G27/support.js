...
{
  aux_text = createDiv("The targets are grouped into their respective inital letters and ordered alphabetically within their groups. \
    Please take your time to look at the layout as the time only starts after selecting the first target.");
  aux_text.id('aux_text');
  aux_text.position(10, height/2 + 50);
  aux_text.style('color', 'rgb(255,66,66)');
  aux_text.style('font-size', '24px');
  aux_text.style('font-weight', 'bold');
  aux_text.style('font-family', 'Arial, sans-serif');
  aux_text.style('text-align', 'center');
  aux_text.style('width', '675px');
  aux_text.style('word-wrap', 'break-word');
}
